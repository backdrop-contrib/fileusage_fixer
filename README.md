...
Go to admin/content/files and sort by "Use count" (ascending). Files without
usage count and in state temporary will automatically get deleted on cron
runs (after a grace time of 6 hours) - that's normal behavior.
Files without usage count, but in state "permanent" will not get deleted,
but they're not actually usable, either.

Trying to use them in the editor via Image library will end up with a form
error "The file used in the XXX field may not be referenced.". And that's
for a good reason.
